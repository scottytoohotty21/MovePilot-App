...
async function supersededSharePreparedListedInventoryPdf() {
    ...
    try {
        // Modern devices
        if (navigator.share) {
            await navigator.share(listedInventoryPreparedSharePayload);

            listedInventoryPreparedSharePayload = null;
            listedInventoryPreparedShareFileName = "";
            closeListedPdfReadyModal();

            await appAlert("PDF shared successfully.", "PDF Shared");
            return;
        }

        // Fallback for older tablets
        const blob = listedInventoryPreparedSharePayload.files[0];
        const url = URL.createObjectURL(blob);

        const fallbackLink = document.createElement("a");
        fallbackLink.href = url;
        fallbackLink.download = listedInventoryPreparedShareFileName || "ListedInventory.pdf";
        ...
        await appAlert(
            "Your device does not support direct sharing. PDF downloaded instead. You can share manually from your downloads.",
            "PDF Downloaded"
        );
    } catch (err) {
        console.error("Prepared PDF share failed:", err);

        if (err && err.name === "AbortError") {
            await appAlert("PDF sharing was cancelled.", "PDF Share Cancelled");
            return;
        }

        await appAlert(
            "The PDF could not be shared: " + (err && err.message ? err.message : err),
            "PDF Share Failed"
        );
    }
}
...
function deleteQuoteAvailabilityDate(sequenceId, dateId) {
    ...
    pricing.dates = pricing.dates.filter(function(row) {
        return row.id !== dateId;
    });
...
function updateQuoteAvailabilityDate(sequenceId, dateId, field, value) {
    ...
    const row = pricing.dates.find(function(item) {
...
async function refreshQuoteAvailabilityFromLocalTable(sequenceId) {
    ...
    if (matchedCount === 0) {
    await appAlert(
        "No availability matches found in the local table yet. You can still select the bands manually.",
        "No Availability Matches"
    );
} else {
    await appAlert(
        "Availability refreshed for " + matchedCount + " date row(s).",
...
function preparePdfDownloadLink(downloadLink, pdfFile) {
    if (!downloadLink || !pdfFile) return;
...
function bindPdfShareButton(pdfFile) {
    ...
    if (!shareBtn || !downloadLink || !pdfFile) return;
